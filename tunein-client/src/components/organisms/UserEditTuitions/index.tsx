...
const EditTuitions: React.FC = () => {
  ...
  useEffect(() => {
    ...
    createDBEndpoint(ENDPOINTS.tutorship.getusertutorships + user.id)
      .post(value)
      .then((res: any) =>
        setTuitions(res.data.tutorships.sort((a: any, b: any) => a.id - b.id))
      );
  }, [user]);
  ...
  return (
    <Styled.Wrapper>
      <Styled.Header>
        <Typography variant="RegisterSuccess" style={{ flex: "0.2" }}>
          Id
        </Typography>
        <Typography variant="RegisterSuccess" style={{ flex: "1" }}>
          Title
        </Typography>
        <Typography variant="RegisterSuccess" style={{ flex: "0.7" }}>
          Category
        </Typography>
        <Typography variant="RegisterSuccess" style={{ flex: "1" }}>
          Price
      ...
      <Styled.List>
        ...
        {tuitions?.map((t) => (
          <li key={t.id} style={{ padding: "0.4rem 0" }}>
            <Styled.IBtn type="button" onClick={() => handleItemClick(t.id)}>
              <Styled.Item variant="EditorList" flexPart={0.2}>
                {t.id}
              </Styled.Item>
              <Styled.Item variant="EditorList" flexPart={1}>
                {t.title}
              </Styled.Item>
              <Styled.Item variant="EditorList" flexPart={0.7}>
                {t.category}
              </Styled.Item>
              <Styled.Item variant="EditorList" flexPart={1}>
                {t.price}
