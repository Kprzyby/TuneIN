...
const VideoHub: React.FC = () => {
  ...
  const { renderInputBar, barInput, setReset, enterEvent } = useInputBar({});

  const enterRoom = () => {
    const pickedVideoCall = activeVideoCalls.find(
      (x: VideoCallType) => x.pin === barInput
    );
      ...
      <Styled.Inner>
        <Styled.Wrapper>
          <Styled.EnterWrapper>
            ...
            <div style={{ paddingTop: "1rem" }}>{renderInputBar}</div>
            <Styled.Button
              type="button"
              onClick={(e) => {
                handleEnterButton(e);
          ...
          <Styled.CreateWrapper>
            <Typography variant="EditorList">Create Video Call</Typography>
            <Styled.Button
              type="button"
              onClick={(e) => {
