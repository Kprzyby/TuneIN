...
const Chat: React.FC<Props> = ({ chatId, messages, isMesseges }) => {
  ...
  const { renderInputBar, barInput, enterEvent, setReset } = useInputBar({});
  ...
  const handleMessageSend = () => {
    createDBEndpoint(ENDPOINTS.chat.sendMessage).post({
      chatId,
      message: barInput,
    });
  };
  ...
  return (
    <Styled.Wrapper>
      <div style={{ width: "100%", height: "100%", position: "relative" }}>
        {!isMesseges ? (
          ...
          <Styled.List>
            {messages?.map((m) => {
              const isSameSender =
                lastSender === undefined ? false : lastSender === m.senderName;
      ...
      <div style={{ width: "100%" }}>{renderInputBar}</div>
    </Styled.Wrapper>
  );
};
